...
export const defaultFieldValues: Record<string, string[]> = {
...
export async function getFieldOptions(): Promise<Record<string, FieldOptionEntry>> {
  await dbConnect();

  const result: Record<string, FieldOptionEntry> = {};

  for (const fieldName of Object.keys(defaultFieldValues)) {
    const row = await PastorFieldOptions.findOne({ fieldName });
    let options: string[] = row ? row.options : defaultFieldValues[fieldName];

    if (fieldName === "clergyTypes") {
      options = options.filter((value) => value !== "Governor");
    }
